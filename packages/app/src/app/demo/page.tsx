import { Players } from '@/components/Players'
import { AttestationFlow } from '@/components/AttestationFlow'
import { VerificationCards } from '@/components/VerificationCards'
import { RevocationDemo } from '@/components/RevocationDemo'
import { WhyItMatters } from '@/components/WhyItMatters'
import { Explorer } from '@/components/Explorer'

export default function DemoPage() {
  return (
    <div className='min-h-screen'>
      {/* Hero for Demo Page */}
      <section className='hero-gradient py-16 px-4 text-center'>
        <h1 className='text-4xl md:text-5xl font-bold mb-4'>
          <span className='gradient-text'>Interactive Demo</span>
        </h1>
        <p className='text-xl text-slate-400 max-w-2xl mx-auto'>
          See how EAS attestations power compliant investor verification for security tokens.
          No wallet needed — everything is simulated.
        </p>
      </section>

      {/* Main Demo Content */}
      <div className='container max-w-5xl mx-auto px-4'>
        {/* Section 1: Meet the Players */}
        <Players />

        <div className='section-divider' />

        {/* Section 2: Attestation Flow */}
        <AttestationFlow />

        <div className='section-divider' />

        {/* Section 3: Verification */}
        <VerificationCards />

        <div className='section-divider' />

        {/* Section 4: Revocation */}
        <RevocationDemo />

        <div className='section-divider' />

        {/* Section 5: Why It Matters */}
        <WhyItMatters />

        <div className='section-divider' />

        {/* Section 6: Explorer */}
        <Explorer />
      </div>

      {/* Bottom CTA */}
      <section className='py-20 px-4 text-center bg-slate-900/50'>
        <div className='max-w-2xl mx-auto'>
          <h2 className='text-3xl font-bold mb-4 text-white'>Want to implement this?</h2>
          <p className='text-slate-400 mb-8'>
            The EAS-ERC3643 bridge is open source and ready for integration with your security token platform.
          </p>
          <a
            href='https://github.com/EntEthAlliance/rnd-rwa-erc3643-eas/'
            target='_blank'
            rel='noopener noreferrer'
            className='btn btn-lg bg-blue-600 hover:bg-blue-500 border-none text-white gap-2 px-8 shadow-lg shadow-blue-600/25'
          >
            <svg className='w-5 h-5' fill='currentColor' viewBox='0 0 24 24'>
              <path d='M12 0c-6.626 0-12 5.373-12 12 0 5.302 3.438 9.8 8.207 11.387.599.111.793-.261.793-.577v-2.234c-3.338.726-4.033-1.416-4.033-1.416-.546-1.387-1.333-1.756-1.333-1.756-1.089-.745.083-.729.083-.729 1.205.084 1.839 1.237 1.839 1.237 1.07 1.834 2.807 1.304 3.492.997.107-.775.418-1.305.762-1.604-2.665-.305-5.467-1.334-5.467-5.931 0-1.311.469-2.381 1.236-3.221-.124-.303-.535-1.524.117-3.176 0 0 1.008-.322 3.301 1.23.957-.266 1.983-.399 3.003-.404 1.02.005 2.047.138 3.006.404 2.291-1.552 3.297-1.23 3.297-1.23.653 1.653.242 2.874.118 3.176.77.84 1.235 1.911 1.235 3.221 0 4.609-2.807 5.624-5.479 5.921.43.372.823 1.102.823 2.222v3.293c0 .319.192.694.801.576 4.765-1.589 8.199-6.086 8.199-11.386 0-6.627-5.373-12-12-12z' />
            </svg>
            View on GitHub
          </a>
        </div>
      </section>
    </div>
  )
}
